...
Output: false
*/
export default function TreeConstructor(strArr: string[]) {
  const parents: { [key: string]: number } = {};
  const childs: { [key: string]: number } = {};

  for (let i = 0; i < strArr.length; i++) {
    const node = strArr[i].replace(/[()]/g, "").split(",");
    const child = node[0];
    const parent = node[1];

    // fill the parents obj
    if (parents[parent]) {
      parents[parent]++;
      if (parents[parent] > 2) return false;
    } else parents[parent] = 1;

    // fill the childs obj
    if (childs[child]) {
      childs[child]++;
      if (childs[child] > 1) return false;
    } else childs[child] = 1;
  }
  return true;
}
